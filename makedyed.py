...
itemscolors = ["RED", "ORANGE", "YELLOW", "LIME", "GREEN", "LIGHT_BLUE", "CYAN", "BLUE", "PINK", "MAGENTA", "PURPLE", "LIGHT_GRAY", "GRAY", "BROWN", "BLACK"]
...
dyefiles = source + "dyed/" + type_ + "/"

rendererfile = dyefiles + "renderers/white_dye_" + type_ + ".java"
for color in colors:
    shutil.copyfile(rendererfile, dyefiles + "renderers/" +  color + "_dye_" + type_ + ".java")
    with open(dyefiles + "renderers/" +  color + "_dye_" + type_ + ".java", 'r') as file:
        data = file.read()
        data = data.replace("white", color)
    with open(dyefiles + "renderers/" +  color + "_dye_" + type_ + ".java", 'w') as file:
        file.write(data)
    print(color + " done")
print("renderers done")

itemfile = dyefiles + "items/White" + ntype + "Item.java"
for color, color2 in zip(itemcolors, itemscolors):
    shutil.copyfile(itemfile, dyefiles + "items/" + color + ntype + "Item.java")
    with open(dyefiles + "items/" + color + ntype + "Item.java", 'r') as file:
        data = file.read()
        data = data.replace("White", color)
        data = data.replace("WHITE", color2)
    with open(dyefiles + "items/" +  color + ntype + "Item.java", 'w') as file:
        file.write(data)      
    print(color + " done")
print("items done")

with open("itemidsadditions.txt", "w+") as file:
    file.write('    public static final ResourceKey<Item> WHITE_DYED_' + type_upper + ' = create("white_dyed_' + type_ + '");\n')
...
with open("screensadditions.txt", "w+") as file:
    file.write("    white_dye_" + type_ + " white_dye_" + type_ + " = new white_dye_" + type_ + "();\n      TrinketRendererRegistry.registerRenderer(Items.WHITE_DYED_" + type_upper + ", white_dye_" + type_ + ");\n")
    for color, color2 in zip(colors, itemscolors):
        file.write(color + "_dye_" + type_ + " " + color + "_dye_" + type_ + " = new " + color + "_dye_" + type_ + "();\nTrinketRendererRegistry.registerRenderer(Items." + color2 + "_DYED_" + type_upper + ", " + color + "_dye_" + type_ + ");\n")
        print(color + " done")
print("Items done")
